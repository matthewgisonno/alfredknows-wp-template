/* global alfredKnowsBgColors, alfredKnowsPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( alfredKnowsBgColors ).forEach( function( context ) {
					alfredKnowsGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function alfredKnowsGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'alfredknows-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#alfredknows-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( alfredKnowsPreviewEls[ context ] ) {
				_.each( alfredKnowsPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		alfredKnowsGenerateColorA11yPreviewStyles( 'content' );
		alfredKnowsGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
